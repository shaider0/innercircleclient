...
class DiscoveryItemMenu extends Component {
  constructor(props){
    super(props)
    this.state = {
      showMenu: false
  ...
  render() {
    const { removeDiscovery, updateDiscovery, isCorrectUser, availableOn, title, description, status, discoveryId, userId, category, currentUser, imageUrl } = this.props
    ...
    const menu = (
      <div className="dropdown" ref={node => this.node = node}>
        ...
        <div className={this.state.showMenu ? shown : hidden}>
          <Link to={{
            pathname: `/users/${userId}/discoveries/${discoveryId}/update`,
            state: {
              title,
              description,
              imageUrl,
          ...
          <Link to="#" onClick={removeDiscovery}>Delete</Link>
        <Link to={{
...
export default DiscoveryItemMenu
